...
window.showTab = function (i) {
  ...
  // само в първия таб имаме селекти → окачваме слушателите
...
function normalizePreviewDOM(){
  const wrap = document.querySelector('.item.has-viewer .visual');
  if(!wrap) return;
...
function initTabs(){
  // просто стартираме първия таб; бутоните вече имат onclick="showTab(...)"
  window.showTab(0);
}
